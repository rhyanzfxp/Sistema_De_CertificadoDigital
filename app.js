...
async function gerarPDF(certMeta, filePath) {
  return new Promise(async (resolve, reject) => {
    ...
    // === NOVO BLOCO PARA GERAR QR DIRETO PARA O PDF ===
    const envBase = (process.env.BASE_URL_EXTERNA || 'http://localhost:3000').trim();
    const base = envBase.replace(/\/$/, '');

    // cria link absoluto direto para download do certificado
    ...
    doc.font("Helvetica").fontSize(9).fillColor("#ffffff")
       .text("Acesse seu certificado direto:", 40, 730);
    doc.fillColor("#00eaff").text(absoluteDownloadUrl, 40, 742, { width: 400 });
...
app.post('/api/certificados/emitir', async (req, res) => {
  try {
    ...
    // gera URLs absolutas baseadas na variável de ambiente
    const basePublic = (process.env.BASE_URL_EXTERNA || 'http://localhost:3000').replace(/\/$/, '');
